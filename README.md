...
- **A legal corpus.** The sample text is five sources chosen so the failure modes discussed in class can be seen in the pipeline: the Fifth Circuit's sanctions order in *Fletcher v. Experian* (Feb. 18, 2026, excerpted), Federal Rule of Civil Procedure 11, Judge Brantley Starr's standing order on generative AI (N.D. Tex.), a fictional engagement letter with a definitions section far from the clause that uses it, and the Fifth Circuit's proposed AI-certification rule from 2023, which the court never adopted. Every chunk is tagged with the document it came from and with a citator-style flag (good law, check current version, not adopted, not an authority).
- **Search settings that mirror commercial tools**, all off by default: hybrid search (keyword matching fused with similarity by reciprocal rank fusion), a cross-encoder reranker (mxbai-rerank-xsmall, in the browser) that rescores the top ten and shows what moved, a filter to leave out flagged sources, and per-source include/exclude. Query words are highlighted in the ranked passages, and flagged passages carry a "Citator:" label into the model's prompt.
